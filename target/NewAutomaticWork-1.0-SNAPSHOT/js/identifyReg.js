...
$(function(){
    ...
    function GetSample() {
        ...
        var $samplePersonTR = $("tr", "#samplePersonTable");
        ...
        var $sampleEvidenceTR = $("tr", "#sampleEvidenceTable");
    ...
    function submitForm() {
        ...
        $.ajax({
            url : "../reg/submit.html?operateType="+operateType,
            type:"post",
...
// sample start
    ...
    function SaveSampleRow() {
        ...
        var newRowHtml = "<td><input type='hidden' name='sampleName' value='"+ $.trim(sampleName)+"'/>"+sampleName+"</td>";
        ...
        if("add" == operateType) {
            ...
            if (operate == 2) {
                var paramStr = "";
                paramStr += "id=" + sampleId;
                paramStr +=  "&sampleType=" + $.trim(sampleTypeCode);
                paramStr +=  "&sampleName=" + $.trim(sampleName);
                paramStr +=  "&extractDatetime=" + extractDatetime;
                paramStr +=  "&extractPerson=" + extractPerson;
                paramStr +=  "&sampleDesc=" + $.trim(sampleDesc);
                paramStr +=  "&samplePacking=" + $.trim(samplePacking);
                paramStr +=  "&sampleProperties=" + $.trim(samplePropertiesVal);
                if(refPersonName != ""){
                    paramStr +=  "&refPersonName=" + refPersonName;
                }
                paramStr +=  "&sampleFlag=" + sampleFlagVal;
                $.ajax({
                    url:"../caseinfo/updateSampleById.html?" + paramStr,
                    type:"get",
                    dataType:"json",
                    success:function(data){
                        if(data.success || data.success == true || data.success == "true") {
                            var consignmentId=$("#consignmentId","#consignment_form").val();
                            location.href = "../caseinfo/edit.html?consignmentId=" + consignmentId;
                        }else {
                            alert("修改失败！");
                        }
                    }
